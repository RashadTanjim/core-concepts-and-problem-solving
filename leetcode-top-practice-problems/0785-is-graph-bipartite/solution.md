# 785. Is Graph Bipartite? - Interview Solution

[Problem description](./README.md) | [Go implementation](./solution.go) | [Java implementation](./Solution.java) | [LeetCode](https://leetcode.com/problems/is-graph-bipartite/)

## Pattern recognition

The main signal is **Coloring Method to Determine Bipartite Graph**. Before coding, state what information the algorithm must remember and why that removes repeated work from the direct approach.

## Approach 1: Brute force

The baseline may launch redundant reachability checks. Build an adjacency or component representation and process each relationship systematically.
...
2. Represent that state with the data structure implied by **Coloring Method to Determine Bipartite Graph**.
...
**Invariant:** Visited/component state exactly represents the vertices already proven reachable or connected under processed edges.
...
Adjacency traversal is `O(V+E)`; union-find is near-linear at `O(E alpha(V))`. Space is `O(V+E)` or `O(V)` respectively. State the bound in terms of the prompt's dimensions (for example <code>n</code>, <code>m</code>, vertices, edges, or generated states) during the interview.
