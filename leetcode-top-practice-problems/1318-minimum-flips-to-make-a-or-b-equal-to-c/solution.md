# 1318. Minimum Flips to Make a OR b Equal to c - Interview Solution

[Problem description](./README.md) | [Go implementation](./solution.go) | [Java implementation](./Solution.java) | [LeetCode](https://leetcode.com/problems/minimum-flips-to-make-a-or-b-equal-to-c/)

## Pattern recognition

The main signal is **Bit Manipulation**. Before coding, state what information the algorithm must remember and why that removes repeated work from the direct approach.

## Approach 1: Brute force

A per-value or per-bit counting baseline uses extra state. Exploit the algebraic identities of XOR, masks, or shifts to combine information directly.
...
2. Represent that state with the data structure implied by **Bit Manipulation**.
...
**Invariant:** After processing each value or bit position, the accumulator represents the required property for the processed prefix.
...
The scan is linear in the number of values, or linear times the fixed machine-word width; auxiliary space is constant. State the bound in terms of the prompt's dimensions (for example <code>n</code>, <code>m</code>, vertices, edges, or generated states) during the interview.
